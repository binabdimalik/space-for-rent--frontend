...
import { FiHome, FiGrid, FiUsers, FiCalendar, FiSearch, FiEye, FiDollarSign, FiMail, FiPhone, FiX, FiMessageCircle } from 'react-icons/fi';
import { AuthContext } from '../../context/AuthContext';
import { BookingsContext } from '../../context/BookingsContext';

const AdminBookings = () => {
    const { user } = useContext(AuthContext);
    const { bookings: contextBookings, updateBookingStatus } = useContext(BookingsContext);
                                    ...
                                    <td>
                                        <div style={{ fontSize: '12px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
                                                ...
                                                <a href={`mailto:${booking.user_email}`} style={{ color: '#2563eb', textDecoration: 'none' }}>{booking.user_email}</a>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                <FiPhone size={12} color="#6a7282" />
                                                <a href={`tel:${booking.user_phone}`} style={{ color: '#1e3a8a', textDecoration: 'none' }}>{booking.user_phone}</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div style={{ fontSize: '13px' }}>
                                            <div style={{ fontWeight: 600 }}>{booking.start_date}</div>
                                            <div style={{ color: '#6a7282' }}>{booking.start_time} - {booking.end_time}</div>
                                        </div>
                                    </td>
                                    <td>
                                        <div style={{ fontSize: '13px' }}>
                                            <div style={{ fontWeight: 700, color: '#2563eb' }}>${booking.total_amount}</div>
                                            <span className={`status-badge ${booking.payment_status === 'paid' ? 'status-available' : 'status-pending'}`} style={{ fontSize: '10px', padding: '2px 6px' }}>
                ...
                <div className="modal-overlay" onClick={() => setSelectedBooking(null)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                            ...
                            <button onClick={() => setSelectedBooking(null)} className="btn btn-ghost btn-sm" style={{ padding: '4px' }}>
                                <FiX size={20} />
                            </button>
                        </div>

                        {/* Space Info */}
                        <div style={{ background: '#f5f5f4', padding: '16px', borderRadius: '8px', marginBottom: '16px' }}>
                            <h3 style={{ fontWeight: 700, color: '#1e3a8a', marginBottom: '4px' }}>{selectedBooking.space_title}</h3>
                            <p style={{ color: '#6a7282', fontSize: '14px' }}>
                                {selectedBooking.start_date} • {selectedBooking.start_time} - {selectedBooking.end_time}
                            </p>
                        </div>

                        {/* Client Info */}
                        <div style={{ marginBottom: '20px' }}>
                            <h4 style={{ fontWeight: 600, color: '#1e3a8a', marginBottom: '12px', fontSize: '14px' }}>
                                <FiUsers size={14} style={{ marginRight: '6px' }} />
                                Client Information
                            </h4>
                            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
                                <p style={{ fontWeight: 600, marginBottom: '8px' }}>{selectedBooking.user_name}</p>
                                <p style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', fontSize: '14px' }}>
                                    <FiMail size={14} color="#2563eb" />
                                    <a href={`mailto:${selectedBooking.user_email}`} style={{ color: '#2563eb' }}>{selectedBooking.user_email}</a>
                                </p>
                                <p style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                                    <FiPhone size={14} color="#059669" />
                                    <a href={`tel:${selectedBooking.user_phone}`} style={{ color: '#059669' }}>{selectedBooking.user_phone}</a>
                                </p>
                            </div>
                        </div>
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                    ...
                                    <span style={{ fontWeight: 700, color: '#2563eb', fontSize: '18px' }}>${selectedBooking.total_amount}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                    <span style={{ color: '#6a7282' }}>Payment Status:</span>
                                    <span className={`status-badge ${selectedBooking.payment_status === 'paid' ? 'status-available' : 'status-pending'}`}>
                                        {selectedBooking.payment_status === 'paid' ? '✓ Paid' : '⏳ Pending'}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                    <span style={{ color: '#6a7282' }}>Payment Method:</span>
                                    <span style={{ fontWeight: 500 }}>{selectedBooking.payment_method}</span>
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span style={{ color: '#6a7282' }}>Payment Date:</span>
                                    <span style={{ fontWeight: 500 }}>{selectedBooking.payment_date}</span>
                                </div>
                            </div>
                        </div>
