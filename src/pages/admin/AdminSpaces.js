...
import { FiHome, FiGrid, FiUsers, FiCalendar, FiPlus, FiEdit2, FiTrash2, FiSearch, FiMapPin, FiImage, FiDollarSign, FiCheck, FiX, FiClock, FiMessageCircle } from 'react-icons/fi';
...
const AdminSpaces = () => {
    ...
    const { spaces, addSpace, updateSpace, deleteSpace, toggleStatus, verifySpace, rejectSpace, getPendingSpaces } = useContext(SpacesContext);
    ...
    const handleVerify = (id) => {
        if (window.confirm('Approve this space listing? It will be visible to all clients.')) {
            verifySpace(id);
        }
    };

    const handleReject = (id) => {
        if (window.confirm('Reject this space listing? It will not be visible to clients.')) {
            rejectSpace(id);
        }
    };
            ...
            <main className="admin-content">
                        ...
                        <tbody>
                            {filteredSpaces.map(space => (
                                <tr key={space.id}>
                                    <td>
                                        <img 
                                            src={space.image_url || 'https://via.placeholder.com/60x40?text=No+Image'} 
                                            alt={space.title}
                                    ...
                                    <td style={{ fontWeight: 600, color: '#2563eb' }}>${space.price_per_hour}</td>
                                    {activeTab === 'pending' ? (
                                        <td>
                                            <div style={{ fontSize: '13px' }}>
                                                <div style={{ fontWeight: 600 }}>{space.submitted_by?.name || 'Unknown'}</div>
                                                <div style={{ color: '#6a7282', fontSize: '12px' }}>{space.submitted_by?.email || ''}</div>
                                                <div style={{ color: '#9ca3af', fontSize: '11px' }}>{space.submitted_at}</div>
                                            </div>
                                        </td>
                                    ) : (
                                        <td>
                                            ...
                                            >
                                                {space.status === 'available' ? '✓ Available' : '✗ Rented'}
                                            </button>
                                        </td>
                                    )}
                                    <td>
                                        {activeTab === 'pending' ? (
                                            <div style={{ display: 'flex', gap: '8px' }}>
                                                <button 
                                                    ...
                                                    style={{ padding: '6px 12px', background: '#059669', color: 'white' }}
                                                    title="Approve"
                                                >
                                                    <FiCheck size={16} />
                                                    Approve
                ...
                {filteredSpaces.length === 0 && (
                    <div className="empty-state">
                        ...
                        <p>{activeTab === 'pending' ? 'No pending spaces to verify.' : 'No spaces found.'}</p>
                    </div>
                )}
            </main>
            ...
            {showModal && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '700px', maxHeight: '90vh', overflow: 'auto' }}>
                        <h2 className="modal-title">{editingSpace ? 'Edit Space' : 'Add New Rental Space'}</h2>
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="form-group">
                                ...
                                <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <p style={{ fontSize: '12px', color: '#6a7282', margin: '8px 0' }}>Or enter image URL:</p>
                                        <input
                                            type="url"
                            ...
                            <div className="form-group">
                                ...
                                <div style={{ height: '200px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #e5e7eb' }}>
                                    <MapContainer 
