...
import { FiHome, FiGrid, FiUsers, FiCalendar, FiDollarSign, FiPlus, FiEdit2, FiTrash2, FiSearch, FiShield, FiMessageCircle } from 'react-icons/fi';
...
const AdminUsers = () => {
    ...
    const { admins, addAdmin, deleteAdmin: deleteAdminFromContext } = useContext(AdminsContext);
    const { clients, deleteClient: deleteClientFromContext } = useContext(ClientsContext);
    ...
    const isSuperAdmin = user?.role === 'super_admin' || user?.email === 'superadmin@spacesforrent.com';
                ...
                {isSuperAdmin && (
                    <div style={{ 
                        ...
                        <span style={{ color: '#92400e', fontSize: '14px' }}>
                            You are logged in as <strong>Super Admin</strong>. You can add and manage other administrators.
                        </span>
                    </div>
                )}
                        ...
                        <table>
                            ...
                            <tbody>
                                {filteredAdmins.length === 0 ? (
                                    <tr>
                                        <td colSpan={isSuperAdmin ? 5 : 4} style={{ textAlign: 'center', color: '#6b7280', padding: '24px' }}>
                                    ...
                                    filteredAdmins.map(adminData => (
                                        <tr key={adminData.id}>
                                            ...
                                            <td>
                                                <span style={{
                                                    background: adminData.role === 'super_admin' ? '#fef3c7' : '#dbeafe',
                                                    color: adminData.role === 'super_admin' ? '#d97706' : '#1d4ed8',
                                                    padding: '4px 10px',
                                                    borderRadius: '12px',
                                                    fontSize: '12px',
                                                    fontWeight: 600
            ...
            {showModal && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        <h2 className="modal-title">{editingUser ? 'Edit Admin' : 'Add New Admin'}</h2>
