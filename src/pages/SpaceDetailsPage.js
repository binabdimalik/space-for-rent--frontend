...
const SpaceDetailsPage = () => {
  ...
  const handleBookingChange = (e) => {
    ...
    // For phone field, only allow valid phone characters
    if (name === 'phone') {
      const cleanValue = value.replace(/[^\d\s\+\-\(\)]/g, '');
      setBookingData({ ...bookingData, [name]: cleanValue });
      return;
    }
  ...
  const validateForm = () => {
    ...
    } else if (!validateName(bookingData.fullName)) {
    ...
    // Validate times for hourly booking
  ...
  const handleBooking = () => {
    ...
    // Validate all fields
    if (!validateForm()) {
      setBookingError('Please correct the errors in the form');
      return;
    }
    ...
    if (conflict.hasConflict) {
      setBookingError(
        `This space is currently unavailable for your selected dates. It has been booked from ${conflict.conflictingBooking.startDate} to ${conflict.conflictingBooking.endDate}. Please explore other available spaces or select different dates.`,
      );
      return;
    }
  ...
  const amenitiesList = space.amenities
    ? space.amenities.split(",").map((a) => a.trim())
    : [];
      ...
      <div className="space-details-container">
        ...
        <div className="space-main-content">
          ...
          <div style={{ position: "relative" }}>
            ...
            />
            <span
              className={`space-badge ${bookingError ? "space-badge-unavailable" : ""}`}
            >
              {bookingError
                ? "✗ Unavailable"
        ...
        <div className="booking-card">
          ...
          <p className="booking-card-subtitle">
            Select your preferred dates and time
          </p>
            ...
            <>
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={handleBookingChange}
                  className={`form-input ${fieldErrors.startTime ? 'input-error' : ''}`}
                />
